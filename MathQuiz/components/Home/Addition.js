...
import { showTime } from '../redux/action';
...
const genQandA = () => {
  ...
  randomOptions = () => Math.round(Math.random() * 200);
...
export default Addition = ({navigation}) => {
  // const setData = async () => {
  //   try {
  ...
  const remainTime = useSelector((state)=> state.reducer);
  //console.warn(remainTime);
...
const Main = props => {
  ...
  useEffect(() => {
    ...
    timer = setInterval(() => {
      ...
      timeRemain = Math.round(time*10)/10;
...
const Options = props => {
  ...
  const timeRemain = props.timeRemain;
  // const dispatch = useDispatch();


  // const handleTime = (time) => {
  //   dispatch(showTime(time));
  // }
